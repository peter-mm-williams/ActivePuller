...
void PairPOL::coeff(int narg, char **arg)
{
  ...
  double f_one = force->numeric(FLERR,arg[2]);
  chain_type = (int) force->numeric(FLERR,arg[3]);
  //int ctype_one = force->numeric(FLERR,arg[3]);
  double k_one = force->numeric(FLERR,arg[4]);
  Npol = (int) force->numeric(FLERR, arg[5]);
  boundlow = (int) force->numeric(FLERR, arg[6]);
  boundhigh = (int) force->numeric(FLERR, arg[7]);
  sigma2  = force->numeric(FLERR, arg[8]);
  double fr_one = force->numeric(FLERR,arg[9]);
  seed = force->inumeric(FLERR,arg[10]); //seed for random number generator. integer
  ...
  if (narg == 8) cut_one = force->numeric(FLERR,arg[6]);
